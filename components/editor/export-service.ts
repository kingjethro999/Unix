...
import { manuscriptToMarkdown } from "@/lib/manuscript-serialization";
...
function markdownRuns(value: string, options: ExportOptions) {
  const pieces = value
    .split(/(\*\*[^*]+\*\*|\*[^*]+\*|`[^`]+`|\[[^\]]+\]\([^)]+\))/g)
    .filter(Boolean);
  return pieces.map((piece) => {
    ...
    const code = piece.startsWith("`") && piece.endsWith("`");
    ...
    return new TextRun({
      ...
      bold,
      italics: italic,
      font: code ? "Courier New" : options.fontFamily || "Calibri",
...
export class ExportService {
  ...
  async exportToPDF(
    ...
    if (options.author) {
      doc.setFontSize(14);
      doc.text(`by ${options.author}`, pageWidth / 2, currentY, {
        align: "center",
      });
      currentY += 10;
    }
    ...
    files.forEach((file, fileIndex) => {
      ...
      const lines = manuscriptToMarkdown(file.document).split("\n");
    ...
    files.forEach((file, fileIndex) => {
      ...
      lines.forEach((line) => {
        if (line.startsWith("# ")) {
          children.push(
            new Paragraph({
              children: markdownRuns(line.substring(2), options),
              heading: HeadingLevel.HEADING_1,
          ...
          children.push(
            new Paragraph({
              children: markdownRuns(line.substring(3), options),
          ...
          children.push(
            new Paragraph({
              children: markdownRuns(line.substring(4), options),
          ...
          children.push(
            new Paragraph({
              children: markdownRuns(line, options),
              spacing: { after: 120 },
            }),
          );
        }
